...
function renderTeamGrid() {
  ...
  teamGrid.querySelectorAll('form').forEach((form) => {
    ...
    input.addEventListener('focus', () => {
      targetDrafts[teamId] = input.value;
    });
  ...
  teamGrid.querySelectorAll('[data-team-reset-id]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      const confirmed = window.confirm(`確定要把 ${teamId} 目前人數歸零嗎？`);
      if (!confirmed) {
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Reset team failed');
        }
      } catch (error) {
        console.error(error);
  ...
  teamGrid.querySelectorAll('[data-team-delta-id]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Team count update failed');
        }
      } catch (error) {
        console.error(error);
        window.alert('加減人數失敗，請稍後再試。');
      } finally {
        button.disabled = false;
      }
    });
...
async function resetAllCountsAndReload() {
  const confirmed = window.confirm('確定要全部歸零嗎？這會把所有桌次目前人數清成 0。');
  if (!confirmed) {
    return;
  }
