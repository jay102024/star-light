...
function render(skipTeamGrid = false) {
  // Update header copy based on mode
  if (mode === 'scoring') {
    heroEyebrow.textContent = '計分模式';
    heroTitle.textContent = '各桌積分排行';
    heroCopy.textContent = '每枚硬幣投入增加 1 分。以下依積分高低排列。';
    panelTitle.textContent = '積分排行';
...
function renderScoringGrid() {
  const sorted = [...teams].sort((a, b) => b.scoreCount - a.scoreCount);

  teamGrid.innerHTML = sorted.map((team, index) => `
    <article class="team-card score-card">
      <div class="team-top">
        <div>
          <p class="eyebrow">#${index + 1} &nbsp;${escapeHtml(team.id)}</p>
